...
""")
  ...
  def test_beeswax_api_autocomplete(self):
    response = _make_query(self.client, "CREATE TABLE nested_table (foo ARRAY<STRUCT<bar:INT, baz:STRING>>)", wait=True)
    ...
    resp = self.client.get(reverse("beeswax:api_autocomplete_tables", kwargs={'database': 'default'}))
    tables = json.loads(resp.content)['tables']
    assert_true("nested_table" in tables)

    # Autocomplete columns for a given table
    resp = self.client.get(reverse("beeswax:api_autocomplete_columns", kwargs={'database': 'default', 'table': 'nested_table'}))
    ...
    resp = self.client.get(reverse("beeswax:api_autocomplete_column", kwargs={'database': 'default', 'table': 'nested_table', 'column': 'foo'}))
    ...
    resp = self.client.get(reverse("beeswax:api_autocomplete_nested", kwargs={'database': 'default', 'table': 'nested_table', 'column': 'foo', 'nested': '$elem$'}))
    json_resp = json.loads(resp.content)
